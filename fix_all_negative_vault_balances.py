#!/usr/bin/env python
"""
Fix ALL negative vault balances across all branches.
This script checks every branch and every vault type (daily and weekly)
and injects capital where needed to bring balances to K0.00.
"""
...
def main():
    print("=" * 80)
    print("FIXING ALL NEGATIVE VAULT BALANCES")
    print("=" * 80)
    ...
    for branch in branches:
        ...
        if daily_vault.balance < 0:
            ...
            VaultTransaction.objects.create(
                ...
                description=f'Capital injection to fix negative balance (recalculation correction)',
            ...
            fixes_made.append(f"{branch.name} - Daily Vault: K{amount:,.2f}")
        else:
        ...
        if weekly_vault.balance < 0:
            ...
            total_injected += amount
            fixes_made.append(f"{branch.name} - Weekly Vault: K{amount:,.2f}")
        else:
    ...
    if fixes_made:
        print(f"\n✅ Fixed {len(fixes_made)} negative vault(s):")
        for fix in fixes_made:
            print(f"   • {fix}")
        print(f"\n💰 Total capital injected: K{total_injected:,.2f}")
    else:
        print("\n✅ No negative balances found - all vaults are OK!")
    
    print("\n✅ All vault balances are now K0.00 or positive")
