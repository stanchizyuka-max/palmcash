...
Shows today's transactions separately to avoid confusion.
"""
...
def main():
    ...
    print(f"ℹ️  Showing transactions BEFORE today separately from today's transactions\n")
    ...
    for branch in branches:
        ...
        # Calculate security balance BEFORE today
        security_in_before = VaultTransaction.objects.filter(
            branch__iexact=branch.name,
            transaction_type='security_deposit',
            direction='in',
            transaction_date__lt=today_start
        ).aggregate(total=Sum('amount'))['total'] or Decimal('0')
        
        security_out_before = VaultTransaction.objects.filter(
            branch__iexact=branch.name,
        ...
        security_balance_before = security_in_before - security_out_before
        
        # Calculate security balance TODAY
        ...
        security_out_today = VaultTransaction.objects.filter(
            branch__iexact=branch.name,
            transaction_type__in=['security_return', 'security_used'],
            direction='out',
            transaction_date__gte=today_start
        ...
        security_balance_today = security_in_today - security_out_today
        
        # Total security balance
        security_balance_total = security_balance_before + security_balance_today
        ...
        print(f"\n🔒 SECURITY DEPOSITS:")
        print(f"   Before Today: K{security_balance_before:>12,.2f} {'✅' if security_balance_before == 0 else '❌'}")
        print(f"   Today:        K{security_balance_today:>12,.2f} [CURRENT OPERATIONS]")
        print(f"   Total:        K{security_balance_total:>12,.2f}")
        ...
        # Check for issues (excluding today's transactions)
        branch_issues = []
        ...
        if security_balance_before != 0:
            branch_issues.append(f"Security deposits (before today): K{security_balance_before:,.2f}")
