...
class UserEditView(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Allow users to edit their own profile, or allow admins/managers/loan_officers to edit any user
        if request.user.id != int(user_id) and request.user.role not in ['admin', 'manager', 'loan_officer'] and not request.user.is_superuser:
